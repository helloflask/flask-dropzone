...
class _Dropzone(object):
    ...
    def config(
        ...
        **kwargs,
        ...
        button_id = kwargs.get(
            "upload_btn_id", current_app.config["DROPZONE_UPLOAD_BTN_ID"]
        )
        in_form = kwargs.get("in_form", current_app.config["DROPZONE_IN_FORM"])
        cancelUpload = kwargs.get(
            "cancel_upload", current_app.config["DROPZONE_CANCEL_UPLOAD"]
        )
        removeFile = kwargs.get(
            "remove_file", current_app.config["DROPZONE_REMOVE_FILE"]
        )
        cancelConfirmation = kwargs.get(
            "cancel_confirmation", current_app.config["DROPZONE_CANCEL_CONFIRMATION"]
        )
        ...
        if click_upload:
            if in_form:
                action = get_url(
                    kwargs.get(
                        "upload_action", current_app.config["DROPZONE_UPLOAD_ACTION"]
                    )
                )
        ...
        allowed_file_type = kwargs.get(
            "allowed_file_type", current_app.config["DROPZONE_ALLOWED_FILE_TYPE"]
        )
        allowed_file_custom = kwargs.get(
            "allowed_file_custom", current_app.config["DROPZONE_ALLOWED_FILE_CUSTOM"]
        )
        ...
        max_files_exceeded = kwargs.get(
            "max_file_exceeded", current_app.config["DROPZONE_MAX_FILE_EXCEED"]
        )
    ...
    def create(action="", csrf=False, action_view="", id="myDropzone", **kwargs):
        ...
        if action:
            ...
            warnings.warn(
                'The argument was renamed to "action" and will be removed in 2.0.'
            )
            action_url = url_for(action_view, **kwargs)

        if csrf:
            warnings.warn(
                "The argument was deprecated and will be removed in 2.0, use DROPZONE_ENABLE_CSRF instead."
            )

        return Markup(
            """<form action="%s" method="post" class="dropzone" id="%s"
...
class Dropzone(object):
    ...
    def init_app(self, app):
        ...
        )  # millisecond, default to 30000 (30 second)
